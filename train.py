...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, debug_from):
    ...
    progress_bar = tqdm(range(first_iter, opt.ambient_iterations), desc="Training progress")
    first_iter += 1
    for iteration in range(first_iter, opt.ambient_iterations + 1):
        ...
        while network_gui.conn != None:
            try:
                ...
                if do_training and ((iteration < int(opt.ambient_iterations)) or not keep_alive):
                    break
        ...
        seperate_render = True if iteration % 100 == 1 else False

        if iteration < args.gaussian_iterations:
            mode = "gaussian"
            train_normal = False

        elif args.gaussian_iterations <= iteration < args.ambient_iterations:
            mode ="ambient"
            train_normal = True

        render_pkg, losses_extra = render(viewpoint_cam, gaussians, pipe, bg, mode=mode, seperate_render=seperate_render, train_normal=train_normal)
        viewspace_point_tensor, visibility_filter, radii =render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
        # Loss
        gt_image = viewpoint_cam.original_image.cuda()
        loss, Ll1 = compute_loss(render_pkg, gt_image, losses_extra, opt, ambient_restrict=False, return_l1=True)
        loss.backward()
        ...
        with torch.no_grad():
            ...
            if iteration == opt.ambient_iterations:
                progress_bar.close()

            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), 
                            gt_image, render_pkg, losses_extra, mode=mode, train_normal=train_normal)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs,
                    gt_image=None, render_pkg={}, losses_extra={}, mode="gaussian", shadow=False, train_normal=False):
    if tb_writer:
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    render_pkg = renderFunc(viewpoint, scene.gaussians, *renderArgs, 
                                            mode=mode, shadow=shadow, seperate_render=True, train_normal=train_normal)[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=list(range(0, 30_000, 500)))
    parser.add_argument("--save_iterations", nargs="+", type=int, default=list(range(3_000, 30_000, 500)))
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[])
    parser.add_argument("--start_checkpoint", type=str, default = None)
    args = parser.parse_args(sys.argv[1:])
    args.save_iterations.append(args.ambient_iterations)
