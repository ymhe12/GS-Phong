...
def getProjectionMatrixCenterShift(znear, zfar, cx, cy, fl_x, fl_y, w, h, s=None):
    top = cy / fl_y * znear
    ...
    if not s == None: # with shear
        P[0, 1] = 2.0 * s * znear / (right - left)
    return P
...
def depth_pcd2normal(xyz):
    ...
    xyz_normal = torch.nn.functional.pad(xyz_normal.permute(2,0,1), (1,1,1,1), mode='constant', value=0).permute(1,2,0)
    return xyz_normal
